...
import { PAGE_CURSOR_MAX_LENGTH, PAGE_LIMIT_DEFAULT, PAGE_LIMIT_MAX } from '@/lib/constants';
import { validateWith } from './body';

// limit は 1〜最大値の整数 (省略時は既定値)、cursor は空でない文字列 (省略可)
const pageQuerySchema = z.object({
  limit: z.coerce.number().int().min(1).max(PAGE_LIMIT_MAX).default(PAGE_LIMIT_DEFAULT),
  cursor: z.string().min(1).max(PAGE_CURSOR_MAX_LENGTH).optional(),
});
