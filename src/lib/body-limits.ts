...
// 実効の段数は形で変わる（実測: Anthropic の `tools[].input_schema` は 30 段まで通り 31 段で 422、
// OpenAI の `tools[].function.parameters` は `function` のぶん 1 段浅く 29 段まで、
// `allOf` / `anyOf` を 1 段ごとに挟む形は 15 段まで）。実在のツール定義でここに届くものは無い。
...
// （実測・64 KiB: 単一の深い鎖は parse 2.04ms で 422、平坦な本文は 3 段あわせて 0.99ms ＝ 約 2 倍）。
// 通す側の最悪形（深さ 64 の櫛）は parse 1.70 ＋ 深さ検査 0.01 ＋ stringify 1.44 ＝ 3.15ms で
// **平坦本文の約 3 倍**。**系統として消すには parse の前に生の文字列を 1 回走査するしかない**
