...
// **残る境界（fail-closed 側に倒れるので塞がない）**: `error.name = ''` のように空にすると、
// 見出しの候補（`: message` / `''`）が構築時に固定された stack の先頭（`Error: message`）と
// 一致しないので、フレームが 1 行も出なくなる（`stackUnparsed: true` が付く）。
// 失われるのは診断だけで、値は 1 バイトも漏れない
