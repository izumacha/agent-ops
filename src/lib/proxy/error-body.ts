...
//       **これ以上締めない理由**: 総語数に上限を置けば塞げるが、その値は 5 にするしかなく
//       （6 語の散文を落とすため）、実在の `param` が既に 5 語に達している
//       （`messages[0].content[0].source.media_type` / `tools[0].custom.format.grammar.definition`）。
//       余裕ゼロの上限は、ベンダーがパスを 1 段深くした時点で診断を黙って消す。
//
