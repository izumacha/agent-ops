...
describe('リクエスト本文の防御', () => {
  ...
  it('本文の上限そのものが 64KiB 以下である (DoS 対策を緩める方向へ動かさない)', () => {
    // 下の 413 の検査はペイロードを定数から作るので、定数を上げるとペイロードも一緒に大きくなり必ず緑になる
    // (「上限が効いていること」は見るが「上限の値」を誰も見ていない)。本文は上限まで丸ごとメモリに載るため、
    // 値を上げると同時接続数 × 上限のヒープを正規利用者が確保させられる。この 64KiB がテストの持つ唯一の値
    expect(JSON_BODY_MAX_BYTES).toBeLessThanOrEqual(64 * 1024);
  });
