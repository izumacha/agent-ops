...
// 接続先が契約テスト専用 DB かの判定 (入口ガード・setupFiles と同じ規則を 1 か所から読む)
import { contractDatabaseProblem } from '../../scripts/lib/contract-database.mjs';

// 明示フラグが無ければ丸ごとスキップする
const ENABLED = process.env.RUN_PRISMA_CONTRACT === '1';

// 接続先が専用 DB であることを確かめる。同じ判定は tests/setup/contract-database-guard.ts が
// 全テストファイルの前にも行うが、TRUNCATE と同じファイルにも置く (設定側の 1 行が消えたとき、
// この呼び出しが最後の砦になる。判定は共有の規則を呼ぶだけなので写しにはならない)
function assertContractDatabase(): void {
  // 駄目な理由 (専用 DB なら null)
  const problem = contractDatabaseProblem(process.env.DATABASE_URL);
  // 専用 DB でなければ走らせない
  if (problem !== null) throw new Error(`契約テストは専用 DB でだけ実行してください: ${problem}`);
}
// テストで発行するトークンの有効期間 (日)
